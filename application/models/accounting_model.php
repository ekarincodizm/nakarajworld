...
class accounting_model extends CI_Model
{
  ...
  public function IncomeAccounting($member_id)
  {
    ...
    $query = $this->AccountingSourceDetail($query,$account_id, $member_id);

    $index = 0;
    $new_query = array();
    foreach ($query as $row) {
      if ($row['source_detail'][0]['member_id'] == $member_id) {
        $new_query[$index] =$row;
        $index++;
      }
    }
  ...
  public function SelectSourceDetail($source_id, $index_id, $source_table, $account_id)
  {
    ...
    if (count($query)!=0) {
      ...
      if (!empty($query[0]['account_id']) && $query[0]['account_id']!="" && $query[0]['account_id']!=0) {
        $account_detail  = $this->Account_model->FindAccountByID($query[0]['account_id']);
      ...
      if (!empty($query[0]['member_id']) && $query[0]['member_id']!="" && $query[0]['member_id']!=0) {
        $member_detail = $this->Member_model->MemberByID($query[0]['member_id']);
