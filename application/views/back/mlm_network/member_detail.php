
<section class="content">
    <div class="container-fluid">
        <div class="row clearfix">
            <div class="col-md-3">
                <div class="card">

                    <div class="body bg-cyan">
                        <div class='row '>

                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                <img alt="140x140" data-src="holder.js/140x140" class="img-circle" src="data:image/svg+xml;base64," data-holder-rendered="true" style="width: 140px; height: 140px;"/>
                            </div>

                        </div>
                        <div class='row '>
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                <h5>
                                    <?php echo $Result['Upline'][0]['member_name']; ?>
                                </h5>
                                <h5>
                                    รหัส <?php echo $Result['Upline'][0]['member_team'].sprintf("%04d", $Result['Upline'][0]['member_level']).$Result['Upline'][0]['member_code']; ?>
                                </h5>
                            </div>


                        </div>
                        <div class='row '>
                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                <a href="<?php echo site_url('/Admin_controller/user_edit_by_id/'.$Result['Upline'][0]['member_id']); ?>" class="btn btn-info waves-effect">แก้ไขข้อมูล</a>
                            </div>


                        </div>
                    </div>
                </div>
            </div>
            <div class='col-md-9'>
                <div class="col-md-12">
                    <div class="alert bg-teal text-center">
                        เลือกตำแหน่งว่าง
                    </div>

                </div>
                <div class='row '>

                    <div class="col-md-12">
                        <?php foreach ($Result['Downline'] as $Downline): ?>
                            <div class="col-md-4">
                                <div class="card">

                                    <div class="body">
                                        <div class='row '>

                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <img alt="140x140" data-src="holder.js/140x140" class="img-circle" src="data:image/svg+xml;base64," data-holder-rendered="true" style="width: 100px; height: 100px;"/>
                                            </div>

                                        </div>
                                        <div class='row '>
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <h5>
                                                    <?php echo $Downline['member_name']; ?>
                                                </h5>
                                                <h5>
                                                    รหัส <?php echo $Downline['member_team'].sprintf("%04d", $Downline['member_level']).$Downline['member_code']; ?>
                                                </h5>
                                            </div>


                                        </div>
                                        <div class='row '>
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <a href="<?php echo site_url('/Admin_controller/search_user_by_id/'.$Downline['member_id']); ?>" class="btn btn-info waves-effect">เพิ่มเติม</a>
                                            </div>


                                        </div>

                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                        <?php if (count($Result['Downline'])<3): ?>
                            <div class="col-md-4">
                                <div class="card bg-grey">

                                    <div class="body">
                                        <div class='row '>

                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <img class="img-circle" src="<?php echo base_url('assets/img/user.png'); ?>" style="width: 100px; height: 100px;"/>
                                            </div>

                                        </div>
                                        <div class='row '>
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <h5>
                                                    ตำแหน่งว่าง
                                                </h5>
                                            </div>
                                        </div>
                                        <div class='row '>
                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
                                                <a href="<?php echo site_url('Admin_controller/three_form_new/'.$Result['Upline'][0]['member_id']); ?>" class="btn bg-pink waves-effect">เลือกตำแหน่งนี้</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        <?php endif; ?>

                    </div>
                </div>


            </div>
